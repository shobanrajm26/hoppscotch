...
export class PersistenceService extends Service {
  ...
  private setupHistoryPersistence() {
    ...
    restHistoryStore.subject$.subscribe(async ({ state }) => {
      await idbStorage.setDB('history', restHistoryKey, JSON.stringify(state))
    })
  ...
  private setupCollectionsPersistence() {
    this.setupRestCollectionsPersistence().then(r => {
      console.log("Return value from setupRestCollectionsPersistence:", r);
    });
  ...
  private async setupRestCollectionsPersistence() {
    try {
      const collections = await idbStorage.getAllCollections()
    ...
    restCollectionStore.subject$.subscribe(async ({ state }) => {
      try {
        ...
        const tx = (await idbStorage.getDB()).transaction("restCollections", "readwrite");
        const store = tx.objectStore("restCollections")

        // Get all existing keys in IndexedDB
        ...
        existingKeysRequest.onsuccess = () => {
          ...
          state.forEach(collection => {
            if (!collection.id) {
              collection.id = generateUniqueId()
              console.warn("Generated 'id' for collection:", collection)
            }
            const serializableCollection = JSON.parse(JSON.stringify(collection))
            store.put(serializableCollection)
          })

          // Delete collections that are no longer in the state
          existingKeys.forEach((key: IDBValidKey) => {
            if (!state.find(collection => collection.id === key)) {
              store.delete(key)
            }
          })
        }
      } catch (error) {
        console.error("Failed to sync collections to IndexedDB:", error)
      }
    })
...
// Function to generate a unique id
  ...
  private setupRESTTabsPersistence() {
    ...
    watchDebounced(
      ...
      async (newRestTabStateData) => {
        await idbStorage.setDB('history' ,restTabStateKey, JSON.stringify(newRestTabStateData));
      },
      { debounce: 500, deep: true }
    )
  }
